...
class BigUpload {
     ...
     */
    const TEMP_DIRECTORY = '../files/publications/temp/';

    /**
     * Directory files will be moved to after the upload is completed
     */
    const MAIN_DIRECTORY = '../files/publications/';
     ...
     */
    public function finishUpload($finalName, $publication_id) {
...
function main($action, $tempName, $finalFileName, $files, $publication_id) {
    ...
    switch ($action) {
        ...
        case 'finish':
            require 'common.php';
//            echo $update_publication = 'UPDATE publications SET publication_document = "' . $finalFileName . '" WHERE id = "' . $publication_id . '"';
//            $mysqli->query($update_publication);
            header("Refresh:0");
            return $bigUpload->finishUpload($finalFileName, $publication_id);
...
try {
    ...
    if (isset($_GET['name'])) {
        ...
        $extension = end($temp);
        $newfilename = $random_number . '_' . date('d_m_Y_H_i_s') . '_' . $date->getTimestamp() . '.' . $extension;
    ...
    $response = main($action, $tempName, $newfilename, $files, $publication_id);
